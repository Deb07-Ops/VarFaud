                ...
                <div class="flex justify-start">
                  <div class="bg-blue-500 rounded-bl-none rounded-xl px-6 py-3 text-gray-700 backdrop-filter backdrop-blur-md bg-opacity-10 ">
                    <div class="test">
                      ...
                      <p>replace(/[&|;$`\\'"\]/g, '\\$&') // Output: rm\ -rf\ \~ <br/>
                      In this code:

The sanitizeInput function uses a regular expression to match potentially dangerous characters commonly used in shell commands.
It replaces these characters with their escaped equivalents using backslashes (\), which prevents them from being interpreted as special characters by the shell.
When applied to the user input "rm -rf ~", it escapes the space ( ) and tilde (~) characters, making the command safe to use.
However, note that input sanitization is context-sensitive and varies depending on how the sanitized data will be used. In some cases, using parameterized queries or context-specific sanitization functions provided by libraries may be more appropriate. Always consider the specific security requirements of your application and consult relevant security guidelines.
                      </p>
